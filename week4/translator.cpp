#include "profile.h"
#include "test_runner.h"
#include <deque>
#include <iostream>
#include <map>
#include <string_view>
#include <vector>

using namespace std;

class Translator {
    map<string_view, string_view> forward_dict;
    map<string_view, string_view> backward_dict;
    deque<string> data;

    string_view AddOnce (string_view s) {
        for (const auto* map_ptr : {&forward_dict, &backward_dict}) {
            const auto it = map_ptr->find(s);

            if (it != map_ptr->end()) {
                return it->first;
            }
        }
        return data.emplace_back(s);
    }
public:
    Translator() = default;

    void Add(string_view source, string_view target) {
        const string_view source_view = AddOnce(source);
        const string_view target_view = AddOnce(target);

        forward_dict[source] = target_view;
        backward_dict[target] = source_view;
    }
    string_view TranslateForward(string_view source) const {
        if (!forward_dict.count(source)) {
            return {};
        }
        return forward_dict.at(source);
    }
    string_view TranslateBackward(string_view target) const {
        if (!backward_dict.count(target)) {
            return {};
        }
        return backward_dict.at(target);
    }
};

void TestSimple() {
    Translator translator;
    translator.Add(string("okno"), string("window"));
    // translator.Add(string("stol"), string("table"));

    // ASSERT_EQUAL(translator.TranslateForward("okno"), "window");
    // ASSERT_EQUAL(translator.TranslateBackward("table"), "stol");
    // ASSERT_EQUAL(translator.TranslateBackward("stol"), "");
}

int main() {
    TestRunner tr;
    RUN_TEST(tr, TestSimple);
    return 0;
}
