...
public:
    ...
    AirportCounter(TIterator begin, TIterator end) {
        TIterator it = begin;
        for (int i = 0; i < airports_c.size(); ++i) {
            airports_c[i].first = static_cast<TAirport>(i);
        }

        while (it != end) {
            ++airports_c[static_cast<size_t>(*it)].second;
            ++it;
        }
    }

    size_t Get(TAirport airport) const {
        return airports_c[static_cast<size_t>(airport)].second;
    }

    void Insert(TAirport airport) {
        ++airports_c[static_cast<size_t>(airport)].second;
    }
    ...
    void EraseAll(TAirport airport) {
        airports_c[static_cast<size_t>(airport)].second = 0;
    }
